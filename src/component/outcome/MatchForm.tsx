...
const MatchForm: React.FC<Props> = ({ matchId, role }) => {
  ...
  const validateStep = () => {
    ...
    if (step === 0) {
      ...
      // Delivery Time: HH:MM 24-hour format
      if (!form.deliveryTime.trim()) newErrors.deliveryTime = "Delivery time is required";
      else if (!/^([01]\d|2[0-3]):([0-5]\d)$/.test(form.deliveryTime))
        newErrors.deliveryTime = "Time must be in 24-hour format HH:MM";

      // Budget Range: positive number with up to 2 decimal places
      if (!form.budgetRange.trim()) newErrors.budgetRange = "Budget range is required";
      else if (!/^\d+(\.\d{1,2})?$/.test(form.budgetRange) || Number(form.budgetRange) <= 0)
        newErrors.budgetRange = "Budget must be a positive number (max 2 decimals)";
    }
    ...
    // Step 2: Logistics
    if (step === 2) {
      if (!form.logisticsMethod.trim()) newErrors.logisticsMethod = "Preferred logistics method is required";
